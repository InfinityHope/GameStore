...
export const getUserData = createAsyncThunk<IUser, string>(
    'user/fetchUserData',
    async (_id) => {
...
export const getUserLibrary = createAsyncThunk<
    ILibraryItem[],
    string
>('user/getUserLibrary', async (_id) => {
    const res = await axiosInstance.get<ILibraryItem[]>(
        `user/library/${_id}`
    )
    return res.data
})

export const getUserOrders = createAsyncThunk<IOrder, string>(
    'user/getUserOrders',
    async (_id) => {
